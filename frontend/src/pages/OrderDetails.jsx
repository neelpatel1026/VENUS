...
const OrderDetails = () => {
  ...
  if (loading) {
    return (
      <div className="tracking-portal-loading-container-luxury">
        <div className="ordersuccess-spinner" />
        <p className="loading-metrics-text-luxury">Retrieving real-time tracking metrics...</p>
      </div>
    );
  }
  ...
  if (!order || !order._id) {
    return (
      <div className="tracking-portal-empty-container-luxury">
        <div className="empty-tracking-illustration-box-luxury">
          <LuTruck className="empty-tracking-icon-vector-luxury" />
        </div>
        <h2 className="empty-tracking-title-luxury">No Shipment Available</h2>
        <p className="empty-tracking-subtitle-luxury">
          You don't have any active shipment to track. Find your favorites in our curated catalog.
        </p>
        
        <div className="empty-tracking-actions-luxury">
          <Link to="/shop" className="btn-tracking-empty-primary-luxury">
            Continue Shopping
          </Link>
          <Link to="/shop" className="btn-tracking-empty-secondary-luxury">
            Browse Collections
          </Link>
        </div>

        {recommendedProducts.length > 0 && (
          <div className="empty-tracking-recs-section-luxury">
            <h4 className="recs-heading-luxury">Trending Skincare For You</h4>
            <div className="recs-grid-luxury">
              {recommendedProducts.map((p) => (
                <div key={p._id} className="rec-product-card-luxury" onClick={() => navigate(`/product/${p._id}`)}>
                  <div className="rec-card-image-luxury">
                    <img src={p.imageUrl || "/cosmetic_1.avif"} alt={p.name} onError={(e) => { e.target.src = "/cosmetic_1.avif"; }} />
                  </div>
                  <div className="rec-card-info-luxury">
                    <h5>{p.name}</h5>
                    <span className="rec-card-price-luxury">₹{p.price.toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="order-details-portal-page-luxury">
      <div className="order-details-portal-max-width">
        
        {/* BREADCRUMB NAVIGATION */}
        <div className="breadcrumb-nav-luxury">
          <Link to="/">Home</Link>
          <LuChevronRight size={12} />
          <Link to="/profile">Account</Link>
          <LuChevronRight size={12} />
          <span className="breadcrumb-current-luxury">Order #{order._id.slice(-8).toUpperCase()}</span>
        </div>
        ...
        <motion.div
          ...
          className="portal-metrics-card-luxury"
        >
          <div className="portal-metrics-header-row">
            <div>
              <div className="portal-metrics-brand-tag">
                <span>VENUS CARE</span>
                <span>•</span>
                <span>TRACKING PORTAL</span>
              </div>
              <h1 className="portal-metrics-title">
                Order #{order._id.slice(-8).toUpperCase()}
              </h1>
              <p className="portal-metrics-date">
          ...
          {/* Header Summary Grid */}
          <div className="portal-metrics-summary-grid">
            ...
            <div className="metrics-summary-block">
              <span className="summary-block-label">AWB Tracking Number</span>
              <div className="summary-block-value-row">
                <strong>VN-{order._id.slice(-8).toUpperCase()}</strong>
                <button onClick={handleCopyTrackingId} title="Copy Tracking ID" className="copy-btn-luxury">
                  <LuCopy size={13} />
                </button>
              </div>
              <span className="summary-block-sub">Courier: Delhivery Logistics</span>
            </div>
            ...
            <div className="metrics-summary-block">
              <span className="summary-block-label">Grand Total</span>
              <strong className="summary-block-value price-highlight">₹{order.totalAmount.toFixed(2)}</strong>
              <span className="summary-block-sub">{order.items?.length || 0} Item(s)</span>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          className="portal-quick-actions-card-luxury"
        >
          <div className="quick-actions-section-title">QUICK ACTIONS & TOOLS</div>
          <div className="quick-actions-buttons-row">
            <button onClick={handleTrackShipment} className="btn-portal-primary-action">
              <LuTruck size={14} /> <span>Track Shipment</span>
            </button>

            <button onClick={downloadInvoice} className="btn-portal-secondary-action">
            ...
            {isReturnPeriodActive && (
              <button onClick={() => navigate(`/return/${order._id}`)} className="btn-portal-danger-action">
                <LuRefreshCw size={14} /> <span>Return Request</span>
              </button>
            )}

            {isCancelable && (
              <button onClick={() => setShowCancelModal(true)} className="btn-portal-danger-action">
                <span>Cancel Order</span>
              </button>
            )}

            <button onClick={handleNeedHelp} className="btn-portal-secondary-action">
              <LuMessageSquare size={14} /> <span>Need Help</span>
            </button>

            <Link to="/shop" className="btn-portal-outline-action">
        ...
        {(order.status === "Cancellation Requested" || order.status === "Cancelled" || order.refundStatus) && (
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            className="portal-timeline-card-luxury"
          >
            <div className="quick-actions-section-title">CANCELLATION & REFUND STATUS</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "24px", marginTop: "16px" }}>
              ...
              {order.cancellationReason && (
                <div>
                  <span className="summary-block-label">Reason for Cancellation</span>
                  <p style={{ margin: 0, fontSize: "14px", color: "#1A1A1A", fontWeight: "600" }}>{order.cancellationReason}</p>
              ...
              {order.refundStatus && (
                <div>
                  <span className="summary-block-label">Refund Details</span>
                  <div style={{ fontSize: "13.5px", color: "#1A1A1A", lineHeight: "1.5" }}>
              ...
              {(order.refundExpectedDate || order.refundDate || order.refundTransactionId || order.refundRemarks) && (
                <div>
                  <span className="summary-block-label">Refund Tracking</span>
        ...
        {/* 3. TRANSIT STATUS TIMELINE */}
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          className="portal-timeline-card-luxury"
        >
          <div className="quick-actions-section-title" id="transit-chronology">DELIVERY TIMELINE & SHIPMENT PROGRESS</div>
        ...
        {/* 4. ITEMS PURCHASED SECTION */}
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          className="portal-items-purchased-card-luxury"
        >
          <div className="quick-actions-section-title">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "20px", marginTop: "16px" }}>
            {(order.items || []).map((item, index) => {
              ...
              return (
                <div key={item._id || index} className="purchased-item-row-luxury">
                  <img 
                    src={item.productImage || "/cosmetic_1.avif"} 
                    alt={item.productName} 
                    className="purchased-item-thumb-luxury"
                  />

                  <div className="purchased-item-info-luxury">
                  ...
                  <div className="purchased-item-actions-luxury">
                    {order.status === "Delivered" && (
                      hasReviewed ? (
                        ...
                        <Link
                          ...
                          className="btn-purchased-review-luxury"
                        >
                          <LuStar size={13} /> Write Review
                        </Link>
                      )
                    )}
                    ...
                    <button
                      ...
                      className="btn-purchased-buy-again-luxury"
                    >
                      Buy Again
                    </button>
                  </div>
        ...
        {/* 5. SPLIT GRID: FINANCIAL SUMMARY & PAYMENT CREDENTIALS */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "28px", marginBottom: "28px" }}>
          ...
          >
            <div className="quick-actions-section-title">FINANCIAL STATEMENT</div>

            <div style={{ display: "flex", flexDirection: "column", gap: "12px", fontSize: "13.5px", marginTop: "16px" }}>
              ...
              <div style={{ borderTop: "1px dashed #ECE7DF", paddingTop: "14px", marginTop: "4px", display: "flex", justifyContent: "space-between", fontSize: "16px", fontWeight: "800", color: "#1A1A1A" }}>
                <span>Grand Total</span>
                <span className="price-highlight">₹{order.totalAmount.toFixed(2)}</span>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            style={{ marginBottom: 0 }}
          >
            <div className="quick-actions-section-title">PAYMENT REGISTRY</div>

            <div style={{ display: "flex", flexDirection: "column", gap: "14px", marginTop: "16px" }}>
              <div>
                <span className="summary-block-label">Payment Method</span>
                <div style={{ fontSize: "14.5px", fontWeight: "700", color: "#1A1A1A", marginTop: "2px" }}>{order.paymentMethod}</div>
              </div>

              <div>
                <span className="summary-block-label">Payment Status</span>
                <div style={{ fontSize: "13.5px", fontWeight: "700", color: order.paymentStatus === "Paid" ? "#16A34A" : "#DC2626", marginTop: "2px" }}>
                  {order.paymentStatus}
                </div>
              </div>
              ...
              {order.paymentId && (
                <div>
                  <span className="summary-block-label">Transaction Reference</span>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginTop: "2px" }}>
                    <span style={{ fontSize: "13px", fontFamily: "monospace", color: "#1A1A1A", fontWeight: "600" }}>{order.paymentId}</span>
                    <button onClick={() => handleCopyTransactionId(order.paymentId)} title="Copy Transaction ID" className="copy-btn-luxury">
                      <LuCopy size={13} />
                    </button>
                  </div>
                </div>
        ...
        {/* 6. SHIPPING & RECIPIENT INFORMATION */}
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          className="portal-items-purchased-card-luxury"
        >
          <div className="quick-actions-section-title">SHIPPING & DESTINATION REGISTRY</div>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "24px", marginTop: "16px" }}>
            <div>
              <span className="summary-block-label">Recipient</span>
              <strong style={{ fontSize: "14.5px", color: "#1A1A1A", display: "block", marginTop: "2px" }}>{order.customerName}</strong>
              <p style={{ margin: "4px 0 0 0", fontSize: "13px", color: "#6B7280" }}>{order.customerPhone}</p>
            </div>

            <div>
              <span className="summary-block-label">Delivery Address</span>
              <p style={{ margin: "2px 0 0 0", fontSize: "13px", color: "#1A1A1A", lineHeight: "1.6" }}>
            ...
            <div>
              <span className="summary-block-label">Logistics Partner</span>
              <strong style={{ fontSize: "14px", color: "#1A1A1A", display: "block", marginTop: "2px" }}>Delhivery Logistics</strong>
              <button 
                onClick={handleCopyTrackingId} 
                className="copy-btn-luxury"
                style={{ fontSize: "12.5px", fontWeight: "600", marginTop: "6px" }}
              >
                Copy Tracking ID
              </button>
            </div>
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="portal-timeline-card-luxury"
          style={{ textAlign: "center", padding: "36px 32px" }}
        >
          <h4 style={{ fontSize: "16.5px", color: "#1A1A1A", margin: "0 0 6px 0", fontWeight: "700", fontFamily: "'Cinzel', serif" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: "24px", flexWrap: "wrap" }}>
            ...
            <Link to="/return-policy" style={{ color: "#C8A165", textDecoration: "none", fontSize: "13px", fontWeight: "700" }}>Return Policy</Link>
            <Link to="/shipping-policy" style={{ color: "#C8A165", textDecoration: "none", fontSize: "13px", fontWeight: "700" }}>Shipping FAQ</Link>
          </div>
        </motion.div>
